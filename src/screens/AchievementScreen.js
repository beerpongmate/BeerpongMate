...
const renderItem = ({ item: { image, name, description, unlocked } }) => (
  <View
    style={[
      ...
      {
        borderColor: unlocked ? cupRed : cupBlue,
        backgroundColor: unlocked ? cupRed : cupBlue
      }
    ]}
  >
    <Image
...
const renderHeader = ({ section: { header } }) => (
  <View style={styles.sectionHeaderContainer}>
    <ThemedText style={styles.sectionHeader}>{header}</ThemedText>
  </View>
);
...
const AchievementsScreen = () => {
  ...
  const [unlocked, locked] = partition(
    achievementData,
    ({ unlocked: unl }) => unl
  );
  const sectionData = [
